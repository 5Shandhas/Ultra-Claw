...
chcp 65001 >nul 2>&1
title Ultra-Claw - Install to Windows

echo.
echo   ========================================
echo     Ultra-Claw 安装到 Windows
echo     从 U 盘离线安装
echo   ========================================
echo.
...
if not exist "%INSTALL_TARGET%\data\.openclaw\openclaw.json" (
    echo {"gateway":{"mode":"local","auth":{"token":"goodforstart"}}} > "%INSTALL_TARGET%\data\.openclaw\openclaw.json"
)

REM ---- Copy HTML pages ----
if exist "%UCLAW_DIR%Config.html" copy "%UCLAW_DIR%Config.html" "%INSTALL_TARGET%\" >nul
if exist "%UCLAW_DIR%Ultra-Claw.html" copy "%UCLAW_DIR%Ultra-Claw.html" "%INSTALL_TARGET%\" >nul
...
echo.
echo cd /d "%%DIR%%core"
echo start /B "" cmd /c "timeout /t 3 /nobreak ^>nul ^&^& start http://127.0.0.1:!PORT!/#token=goodforstart"
