...
if not exist "%STATE_DIR%\openclaw.json" (
    echo {"gateway":{"mode":"local","auth":{"token":"goodforstart"}}} > "%STATE_DIR%\openclaw.json"
)

REM Read token from config
set "TOKEN=goodforstart"
if exist "%STATE_DIR%\openclaw.json" (
    for /f "tokens=*" %%t in ('"%NODE_BIN%" -e "try{const c=JSON.parse(require('fs').readFileSync(process.argv[1],'utf8'));console.log((c.gateway&&c.gateway.auth&&c.gateway.auth.token)||'goodforstart')}catch(e){console.log('goodforstart')}" "%STATE_DIR%\openclaw.json"') do set "TOKEN=%%t"
)
...
if exist "%USERPROFILE%\.uclaw" (
    ...
    echo     Open Settings - Apps - find Ultra-Claw - Uninstall
)
pause
goto :menu
